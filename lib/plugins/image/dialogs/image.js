							...
							[
								{
									...
									[
										...
										},
										{
											...
											hidden : ($("<DIV />").ontraport_components_carousel === undefined),
											onClick: function()
											{
												...
												e.ontraport_components_carousel();
												
												var $dialog = $("<DIV />").ontraport_components_dialog
												(
													{
														title:  $l("Insert Image"),
														attachElement: e,
														width:915,
														minHeight:588,
														theme:'green',
												...
												//style the header
												var dialogWrapper = $dialog.closest('.ussr-dialog')
												//add class for image dialog specific styles
												dialogWrapper.addClass('ussr-image-dialog')	
												//ADD CSS FRAMEWORK STYLES
												//dialog header classes
												dialogWrapper.find('.ussr-dialog-header').addClass('ussr-border-inset-bottom h3').prepend("<span class='ussr-icon ussr-icon-image '></span>");
												
												
												e.bind("menuSelect",
													function(ev,vals,imgs)
													{
														if(imgs)
														{
															self.getDialog().getContentElement('info','txtUrl').setValue(jQuery(imgs[0]).data("primary_src"));
															$dialog.ontraport_components_dialog('close');
														}
													}
												);
												e.bind("menuCancel",
