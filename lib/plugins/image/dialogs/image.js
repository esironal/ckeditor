...
*/
if(typeof global_image_box == 'undefined')
{
	var global_image_box = null;
}
(function()
{
							...
							[
								{
									...
									[
										...
										},
										{
											...
											align : 'center',
											label : 'Select Hosted Image',
											hidden : (typeof ImageBox != "function"),
											onClick: function()
											{
												var urlBox = this.getDialog().getContentElement('info','txtUrl');
												var self = this;
												if(global_image_box==null)
												{
													global_image_box = ImageBox.prototype.GetImageBox("ajax_process.php?action=list_images",null,"include/Boxes/box_process.php?action=add_image");
													
												}
												global_image_box.onclick = function(data)
												{
														var src = data['src'];
														src = replaceAll(src,"https://"+window.location.hostname,"http://images.moon-ray.com");
														src = replaceAll(src,"http://"+window.location.hostname,"http://images.moon-ray.com");
							
							
														src = replaceAll(src,"/designer_files/","/");
														src = replaceAll(src,"/pub_designer_files/","/");
														self.getDialog().getContentElement('info','txtUrl').setValue(src);
														this.Clear();
														$(".cke_dialog_background_cover").show();
														self.getDialog().getElement().show();
												
												}
												global_image_box.onclear = function()
												{
														$(".cke_dialog_background_cover").show();
														self.getDialog().getElement().show();
												}
												$(".cke_dialog_background_cover").hide();
												this.getDialog().getElement().hide();
												global_image_box.Pop();
											}
										}
									]
								}
