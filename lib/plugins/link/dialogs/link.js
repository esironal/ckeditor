                                ...
                                [
                                    ...
                                    },
                                    {
                                        ...
                                        "onClick": function() {
                                            ...
                                            var dialog = ontraport.Components.Carousel.showAsDialog(
                                                {
                                                    "documentMode": true
                                                },
                                                {
                                                    "title": $l("Attachment Files")
                                                });

                                            dialog.bind("menuSelect",
                                                function(ev, vals, files)
                                                {
                                                    var dlink = files[0].download_link;
                                                    
                                                    if (dlink)
                                                    {
                                                        var lcase = dlink.toLowerCase();
                                                    
                                                        if (dlink.length > 7 && lcase.substr(0, 7) == "http://")
                                                        {
                                                            dlink = dlink.substr(7);
                                                        }
                                                        else if(dlink.length > 8 && lcase.substr(0, 8) == "https://")
                                                        {
                                                            dlink = dlink.substr(8);
                                                        }

                                                        self.getDialog().getContentElement("info", "url").setValue(dlink);
                                                    }

                                                    dialog.ontraport_components_dialog("close");

                                                }).bind("menuCancel", function()
                                                {
                                                    dialog.ontraport_components_dialog("close");
                                                });
